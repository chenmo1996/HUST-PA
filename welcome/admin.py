...
from .models import NewMember, Group1, Group2


class NewMemberAdmin(admin.ModelAdmin):
    list_display = ('name', 'sex', 'tel', 'email', 'college', 'dormitory', 'group1','group2','introduction')
    list_filter = ('sex','dormitory','group1','group2')
    fieldsets = (
        ('个人信息', {'fields': ('name', 'sex', 'college', 'dormitory')}),
        ('联系方式', {'fields': ('tel', 'email')}),
        ('分组意向&自我介绍', {'fields': ('group1','group2', 'introduction')}),
    )
